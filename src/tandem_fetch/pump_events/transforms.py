...
try:
    from static_dicts import ALARMS_DICT, ALERTS_DICT, CGM_ALERTS_DICT
except ImportError:
    from .static_dicts import ALARMS_DICT, ALERTS_DICT, CGM_ALERTS_DICT
...
def transform_enum(event_def, name, name_fmt, field, tx):
    ...
    out += [f"    {enumNameFormat(v)} = {k}" for k, v in tx.items() if enumNameFormat(v)]
    out += [""]
    out += [
        "@property",
        f"def {name_fmt}(self):",
...
def transform_bitmask(event_def, name, name_fmt, field, tx):
    ...
    out += [""]
    out += [
        f"class {enumNameFormat(name_fmt)}Bitmask(IntFlag):",
    ]
    out += [f"    {enumNameFormat(v)} = 2**{k}" for k, v in tx.items() if enumNameFormat(v)]
    out += [""]
    out += [
        "@property",
        f"def {name_fmt}(self):",
...
def transform_ratio(event_def, name, name_fmt, field, tx):
    out = []
    out += ["@property", f"def {name_fmt}(self):", f"    return self.{name_fmt}Raw * {tx}", ""]
